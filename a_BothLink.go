...
func main() {
	...
	for _, val := range nodeSet {
		for i, _ := range IDs {
			if val.NodeID != IDs[i] {
				val.routingtable.Update(IDs[i])
			}
		}
		//fmt.Printf("%d\n", val.routingtable.Size())
	}
	// set up for Kademkadem
...
func System(wait *sync.WaitGroup) {
	...
	for Cnt_Reboot <= Limit_Reboot {
		...
		End_Time = time.Now().UnixNano()
		fmt.Printf("Time: %d / Block: %d  / Tx: %d / Attack_T: %d / Attack_S: %d\n", End_Time-Start_Time, BlockCnt, TransactionCnt, Attack_S+Attack_F, Attack_S)
...
func NewNode(index int) *Node {
	...
	if index%10 == 0 {
		...
		/*if index == 0 {
		     ...
		     returnNode.blockchain = CopyBlockchain(returnNode.Num, 0, nodeSet[0].blockchain.Length)
		  }
...
func (node *Node) Donation() {
	if node.Num == 0 {
		...
		node.blockchain = CreateBlockchain(address, node.Num)
		...
		node.blockchain = CopyBlockchain(node.Num, (node.Num/10-1)*10, nodeSet[(node.Num/10-1)*10].blockchain.Length)
		node.utxoSet = UTXOSet{node.blockchain}
		node.utxoSet.Reindex()
		for i := 1; i < 10; i++ {
			node.Send(node.Num-i, subsidy/2)
			node.MiningBlock()
		}
	}
}
...
func (node *Node) BasicUpdate() {
	node.blockchain = CopyBlockchain(node.Num, Num_Basic+Num_Attacker, nodeSet[Num_Basic+Num_Attacker].blockchain.Length)
	node.utxoSet = UTXOSet{node.blockchain}
	node.utxoSet.Reindex()
}
...
func (node *Node) Run(wait *sync.WaitGroup) {
	...
	for Cnt_Reboot <= Limit_Reboot {
		...
		if node.Num%10 == 0 && node.Num != 0 {
			for i := 1; i < 10; i++ {
				if len(sigSet[node.Num-i][node.Num]) > 0 {
					<-sigSet[node.Num-i][node.Num]
		...
		if node.active { // node is on
			...
			if Switch.Cmp(CompareOn) < 0 {
				...
				if node.blockchain == nil {
					...
					sigSet[node.Num][(node.Num/10+1)*10] <- true
					for len(chanSet[(node.Num/10+1)*10][node.Num]) == 0 {
						time.Sleep(10 * time.Millisecond)
					}

					from := <-chanSet[(node.Num/10+1)*10][node.Num]
...
func (node *Node) AddLinkbyKadem() {
	if node.out < Num_LinkOut {
		...
		if node.attacker {
			willLink = targetNode
			IDofwillLink = nodeSet[willLink].NodeID
			//fmt.Printf("%d->%d, Linking by Attacker Setup...\n", node.Num, willLink)
		} else {
			RandomP, _ := rand.Int(rand.Reader, big.NewInt(int64(10)))

			if RandomP.Cmp(big.NewInt(3)) > 0 {
				//fmt.Printf("%d->%d, Linking by RoutingTable Setup...\n", node.Num, willLink)
				...
				//fmt.Printf("%d->%d, Linking by Random Setup..\n", node.Num, willLink)
				time.Sleep(10 * time.Millisecond)
			}
		}
...
func (node *Node) MakeTransaction() {
	...
	/*if node.Num % 10 == 0 && int64(balance)-usingAmount < remainOff[node.Num/10]*subsidy/2 {
	   return
	}*/
...
func (node *Node) UpdateDB() {
	...
	for len(node.forUpdate) != 0 {
		...
		if node.blockchain.Length < tmpNd.blockchain.Length {
			Group[node.Num] = Group[tmpNd.Num/(Num_Basic+Num_Attacker+1)-1]
...
func (node *Node) Alarm() {
	for _, val := range node.link {
		//fmt.Println(val)
		//fmt.Printf("Node %d alarms Node %d: %d\n", node.Num, val, len(chanSet[node.Num][val]))

		if val != node.Num && nodeSet[val].active && nodeSet[val].blockchain != nil && len(chanSet[node.Num][val]) == 0 {
...
func (bc *Blockchain) FindEveryUTXO() map[string]TXOutputs {
	...
	for {
		...
		for _, tx := range block.Transactions {
			txID := hex.EncodeToString(tx.Num)
...
func (b *Block) HashTransactions() []byte {
	...
	for _, tx := range b.Transactions {
		txHashes = append(txHashes, tx.Num)
	}
...
func (tx *Transaction) Sign(privateKey ecdsa.PrivateKey, prevTxs map[string]Transaction) {
	...
	for inID, vin := range txCopy.Vin {
		...
		txCopy.Num = txCopy.Hash()

		r, s, err := ecdsa.Sign(rand.Reader, &privateKey, txCopy.Num)
...
func (tx *Transaction) TrimmedCopy() Transaction {
	...
	txCopy := Transaction{tx.Num, inputs, outputs}
	return txCopy
}
...
func (tx *Transaction) Verify(prevTXs map[string]Transaction) bool {
	...
	for inID, vin := range tx.Vin {
		...
		if ecdsa.Verify(&rawPubKey, txCopy.Num, &r, &s) == false {
			return false
		}
	}
	return true
...
func (bc *Blockchain) FindTransaction(Num []byte) (Transaction, error) {
	bci := bc.Iterator()
	for {
...
func (bc *Blockchain) SignTransaction(tx *Transaction, privKey ecdsa.PrivateKey) {
	...
	for _, vin := range tx.Vin {
		...
		prevTXs[hex.EncodeToString(prevTX.Num)] = prevTX
		if err != nil {
...
func (u UTXOSet) Update(block *Block) {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		...
		for _, tx := range block.Transactions {
			...
			err := b.Put(tx.Num, newOutputs.Serialize())
			if err != nil {
